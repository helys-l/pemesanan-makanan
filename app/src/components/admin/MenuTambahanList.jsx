...
import EditMakananForm from "../elements/EditMenuTambahanForm.jsx";
import AddMakananForm from "../elements/AddMenuTambahanForm.jsx"; // 
...
export default function MakananListFirebase() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const col = collection(db, "menuTambahan");
...
const handleDelete = async (item) => {
      ...
      <div className="mt-2 flex justify-end gap-2">
        ...
        <button
          onClick={async () => {
            ...
            try {
              await deleteDoc(doc(db, "menuTambahan", item.id));
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl md:text-2xl font-bold text-[#DC2318] mb-4">Menu Tambahan</h2>
