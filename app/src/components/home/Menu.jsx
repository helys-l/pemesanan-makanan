...
export default function Menu({ selectedMenu, setSelectedMenu }) {
    ...
    return (
        <header className="container rounded-full h-12 mx-auto sm:h-16 md:h-20 card justify-between gap-x-3 items-center flex px-4 ">
            <div className="h-10 hidden sm:h-8 md:h-12 aspect-[4/1] bg-[#DC2318] text-[#FDFDFE] rounded-full text-sm sm:text-md md:text-lg lg:text-xl font-extrabold justify-center items-center sm:flex">Mas Hely</div>
            <div className="h-10 md:h-12 sm:gap-x-3 gap-x-6 spect-[3/1] md:w-1/2 flex">
                <button
                    onClick={() => setSelectedMenu('makanan')}
                    className={`h-[95%] w-1/3 rounded-full duration-300 font-black text-xs group sm:text-md lg:text-lg gap-2 border flex justify-center items-center ${selectedMenu === 'makanan' ? 'bg-[#FDFDFE] text-[#DC2318] border-[#dc2318]' : 'text-stone-600 border-[#fdfdfe] hover:text-[#DC2318] hover:border-[#dc2318]'}`}
                >
